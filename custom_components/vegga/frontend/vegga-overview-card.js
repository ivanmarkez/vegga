  ...
  _render() {
    ...
    const mobileCards = viewModel.map((item) => `<article class="sector-mobile ${item.order ? "today" : ""}">
      <div class="mobile-head">
        ...
        ${item.delta.cls !== "neutral" ? `<span class="mobile-delta-check ${item.delta.cls}" title="Diferencia ${this._escape(item.delta.text)}" aria-label="Diferencia ${this._escape(item.delta.text)}"><ha-icon icon="mdi:check-circle"></ha-icon><span class="delta-text">${this._escape(item.delta.text)}</span></span>` : ""}
      </div>
