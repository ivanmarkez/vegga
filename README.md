# VEGGA Agrónic para Home Assistant — v0.4.54
...
- Relaciona los sectores activos con `programSector` del detalle del programa.
  Esta es la misma fila con `xState=1` que VEGGA utiliza para mostrar `S27`,
  y sirve cuando el endpoint general de sectores omite `xProgramN`.

## 0.4.53

- Cada selector y tarjeta de sector muestra si está regando y el número y
  nombre del programa que lo mantiene activo, también cuando el modo del
  sector continúa indicando `Automático`.

## 0.4.52

- Añade los condicionantes C1 de baja presión y C2 de alta presión como
  sensores binarios de problema.
- Añade `Paro definitivo por condicionante`, activo cuando actúa un
  condicionante configurado con la función de paro definitivo.
- Expone estado, acción, referencia, valor en crudo y fuera de servicio.

## 0.4.51

- Usa el campo correcto del A-5500, `xValor`, como cantidad pendiente.
- Replica el cálculo oficial: `transcurrido = value - xValor`.
- La tarjeta muestra sector activo, transcurrido/total y tiempo restante.
...
- Corrige el formato específico del A-5500: el detalle guarda sus filas de
  riego en `programSector`.
- Selecciona la fila activa con `xState=1` y muestra su `xValue` pendiente,
  siguiendo la misma lógica de la pantalla oficial de VEGGA.
